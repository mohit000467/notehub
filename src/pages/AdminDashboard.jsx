              ...
              {filteredNotes.length === 0 ? (
                ...
                return (
                  <div key={note.id} className={`grid grid-cols-12 px-4 py-3.5 items-center hover:bg-surface-hover transition-colors ${idx !== filteredNotes.length - 1 ? "border-b border-surface-border" : ""}`}>
                    ...
                    <div className="col-span-1 text-center"><span className="text-xs text-accent-cyan">{note.downloadCount || 0}</span></div>
                    <div className="col-span-2"><p className="text-xs text-gray-600">{formatDate(note.createdAt)}</p></div>
                    <div className="col-span-1 flex justify-center">
                      <button onClick={() => setDeleteTarget(note)} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">
              ...
              {filteredUsers.map((user, idx) => (
                <React.Fragment key={user.id}>
                  <div className={`grid grid-cols-12 px-4 py-3.5 items-center hover:bg-surface-hover transition-colors cursor-pointer ${idx !== filteredUsers.length - 1 ? "border-b border-surface-border" : ""}`}
                    ...
                    <div className="col-span-1 text-center"><span className="text-xs text-ink-400 font-semibold">{getUserNoteCount(user.id)}</span></div>
                    <div className="col-span-1 text-center"><span className="text-xs text-accent-cyan">{getUserDownloads(user.id)}</span></div>
                    <div className="col-span-1 text-center">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${user.profileVisibility === "public" ? "bg-accent-green/10 text-accent-green" : "bg-gray-500/10 text-gray-500"}`}>
                        {user.profileVisibility}
                      </span>
                    </div>
                    <div className="col-span-1 flex justify-center">
                      {expandedUser === user.id ? <ChevronUp size={14} className="text-gray-500" /> : <ChevronDown size={14} className="text-gray-500" />}
                    </div>
                  </div>
                  {expandedUser === user.id && (
                    <div className="bg-surface-elevated border-b border-surface-border px-4 py-4">
                      <p className="text-xs text-gray-500 mb-3 font-semibold uppercase tracking-wide">Notes by {user.username}</p>
                            ...
                            return (
                              <div key={note.id} className="flex items-center justify-between bg-surface-card rounded-xl px-4 py-3 border border-surface-border">
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <a href={note.fileURL} target="_blank" rel="noreferrer" className="p-1.5 text-gray-600 hover:text-ink-400 rounded-lg transition-all"><Eye size={14} /></a>
                                  <button onClick={() => setDeleteTarget(note)} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"><Trash2 size={14} /></button>
                                </div>
                              </div>
                            );
                          })}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface-card border border-surface-border rounded-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setDeleteTarget(null)} className="text-gray-600 hover:text-white"><X size={18} /></button>
            </div>
            ...
            <p className="text-xs text-gray-600 mb-6">By {deleteTarget.uploaderName || deleteTarget.username} · {deleteTarget.subjectDisplay}</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 border border-surface-border text-gray-400 hover:text-white rounded-xl text-sm font-medium transition-colors">Cancel</button>
              <button onClick={handleDelete} disabled={deleting} className="flex-1 py-2.5 bg-red-500 hover:bg-red-400 text-white rounded-xl text-sm font-semibold transition-colors disabled:opacity-60">
