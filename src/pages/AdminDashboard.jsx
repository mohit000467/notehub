  ...
  return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-surface-card border border-surface-border rounded-2xl p-8">
        <div className="w-12 h-12 bg-ink-500/20 rounded-2xl flex items-center justify-center mx-auto mb-5"><Shield size={24} className="text-ink-400" /></div>
        ...
        <form onSubmit={handleSet} className="space-y-4">
          <div className="relative">
            ...
            <input type={show ? "text" : "password"} value={newPass} onChange={(e) => { setNewPass(e.target.value); setError(""); }} placeholder="New admin password" autoFocus className="w-full bg-surface-elevated border border-surface-border rounded-xl pl-9 pr-10 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" />
            <button type="button" onClick={() => setShow(!show)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-300">{show ? <EyeOff size={14} /> : <Eye size={14} />}</button>
          </div>
          <div className="relative">
            <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
            <input type={show ? "text" : "password"} value={confirmPass} onChange={(e) => { setConfirmPass(e.target.value); setError(""); }} placeholder="Confirm password" className="w-full bg-surface-elevated border border-surface-border rounded-xl pl-9 pr-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" />
          </div>
          {error && <p className="text-xs text-red-400 flex items-center gap-1"><AlertTriangle size={12} />{error}</p>}
          <button type="submit" disabled={loading} className="w-full py-3 bg-ink-500 hover:bg-ink-400 text-white font-semibold rounded-xl transition-colors text-sm disabled:opacity-60">{loading ? "Saving..." : "Set Password & Enter"}</button>
        </form>
      </div>
    </div>
  );
...
const AdminLoginGate = ({ onSuccess, correctPassword }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (password === correctPassword) { sessionStorage.setItem("admin_auth", "true"); onSuccess(); }
    else { setError("Incorrect password"); setPassword(""); }
  };
  const handleForgotPassword = async () => {
    setForgotLoading(true);
    try { await sendPasswordResetEmail(auth, ADMIN_EMAIL); setForgotSent(true); toast.success("Firebase reset link sent! 📧"); }
  ...
  const handleChangeAdminPass = async (e) => {
    ...
    if (result.success) { toast.success("Password updated! 🔐"); setShowChangePass(false); setNewPass(""); setConfirmNewPass(""); setChangeError(""); window.location.reload(); }
    else setChangeError("Failed. Try again.");
  };
  if (showChangePass) return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-surface-card border border-surface-border rounded-2xl p-8">
        <button onClick={() => setShowChangePass(false)} className="text-xs text-gray-500 hover:text-white mb-6 flex items-center gap-1 transition-colors">← Back</button>
        ...
        <form onSubmit={handleChangeAdminPass} className="space-y-4 mt-4">
          <div className="relative"><Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" /><input type={show ? "text" : "password"} value={newPass} onChange={(e) => { setNewPass(e.target.value); setChangeError(""); }} placeholder="New password" autoFocus className="w-full bg-surface-elevated border border-surface-border rounded-xl pl-9 pr-10 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" /><button type="button" onClick={() => setShow(!show)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-300">{show ? <EyeOff size={14} /> : <Eye size={14} />}</button></div>
          <div className="relative"><Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" /><input type={show ? "text" : "password"} value={confirmNewPass} onChange={(e) => { setConfirmNewPass(e.target.value); setChangeError(""); }} placeholder="Confirm new password" className="w-full bg-surface-elevated border border-surface-border rounded-xl pl-9 pr-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" /></div>
          {changeError && <p className="text-xs text-red-400 flex items-center gap-1"><AlertTriangle size={12} />{changeError}</p>}
          <button type="submit" disabled={changeLoading} className="w-full py-3 bg-ink-500 hover:bg-ink-400 text-white font-semibold rounded-xl transition-colors text-sm disabled:opacity-60">{changeLoading ? "Updating..." : "Update Password"}</button>
        </form>
      </div>
    </div>
  );
  return (
    <div className="min-h-screen bg-surface-base flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-surface-card border border-surface-border rounded-2xl p-8">
        <div className="w-12 h-12 bg-red-500/10 rounded-2xl flex items-center justify-center mx-auto mb-5"><Shield size={24} className="text-red-400" /></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative"><Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" /><input type={show ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }} placeholder="Enter admin panel password" autoFocus className="w-full bg-surface-elevated border border-surface-border rounded-xl pl-9 pr-10 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" /><button type="button" onClick={() => setShow(!show)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-300">{show ? <EyeOff size={14} /> : <Eye size={14} />}</button></div>
          {error && <p className="text-xs text-red-400 flex items-center gap-1"><AlertTriangle size={12} />{error}</p>}
          <button type="submit" className="w-full py-3 bg-ink-500 hover:bg-ink-400 text-white font-semibold rounded-xl transition-colors text-sm">Access Admin Panel</button>
        </form>
        <div className="mt-5 pt-4 border-t border-surface-border flex flex-col gap-2">
          <button onClick={() => setShowChangePass(true)} className="flex items-center justify-center gap-2 text-xs text-gray-500 hover:text-ink-400 transition-colors"><Settings size={12} /> Change admin panel password</button>
          {forgotSent ? <p className="text-xs text-accent-green text-center flex items-center justify-center gap-1"><CheckCircle size={12} /> Reset link sent!</p>
            : <button onClick={handleForgotPassword} disabled={forgotLoading} className="text-xs text-gray-500 hover:text-gray-400 transition-colors disabled:opacity-50 text-center">{forgotLoading ? "Sending..." : "Forgot Firebase login password?"}</button>}
        </div>
      </div>
    </div>
  );
...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="bg-surface-card border border-surface-border rounded-2xl p-5 flex items-center gap-4">
    ...
    <div><p className="text-2xl font-display font-bold text-white">{value}</p><p className="text-xs text-gray-500">{label}</p></div>
  </div>
);
...
const LockedUserRow = ({ lockedUser, onUnlock }) => {
  ...
  return (
    <div className="grid grid-cols-12 px-4 py-3.5 items-center hover:bg-surface-hover transition-colors border-b border-surface-border last:border-0">
      <div className="col-span-5"><p className="text-sm text-white font-mono">{lockedUser.email}</p></div>
      <div className="col-span-2 text-center"><span className="text-xs font-bold text-red-400">{lockedUser.attempts}</span><p className="text-xs text-gray-600">attempts</p></div>
      <div className="col-span-3">
        <div className="flex items-center gap-1.5"><Clock size={12} className="text-amber-400 flex-shrink-0" /><span className="text-sm font-mono text-amber-400 font-bold">{remaining > 0 ? formatTimeLeft(remaining) : "Expired"}</span></div>
        <p className="text-xs text-gray-600 mt-0.5">remaining</p>
      </div>
      <div className="col-span-2 flex justify-end">
        <button onClick={() => onUnlock(lockedUser)} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500/10 hover:bg-green-500/20 text-green-400 border border-green-500/20 rounded-lg text-xs font-medium transition-all">
...
// ── User Profile Modal — with Joined date + Block/Unblock both ──
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-surface-card border border-surface-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-start justify-between p-6 border-b border-surface-border">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {/* ✅ Joined date in modal */}
              <div className="flex items-center gap-1.5 mt-2">
                <Calendar size={12} className="text-ink-400" />
                <span className="text-xs text-gray-500">
                  Joined: <span className="text-ink-300 font-semibold">
          ...
          <button onClick={onClose} className="text-gray-600 hover:text-white transition-colors mt-1"><X size={20} /></button>
        </div>
        ...
        {/* Notes */}
        <div className="p-6">
          <p className="text-xs text-gray-500 mb-3 uppercase tracking-wide font-semibold">Uploaded Notes ({userNotes.length})</p>
          {userNotes.length === 0 ? <p className="text-sm text-gray-600 italic">No notes uploaded.</p> : (
                ...
                return (
                  <div key={note.id} className="flex items-center justify-between bg-surface-elevated rounded-xl px-4 py-3 border border-surface-border">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                      ...
                      <a href={note.fileURL} target="_blank" rel="noreferrer" className="p-1.5 text-gray-600 hover:text-ink-400 rounded-lg transition-all"><ExternalLink size={13} /></a>
                      <button onClick={() => onDeleteNote(note)} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"><Trash2 size={13} /></button>
                    </div>
                  </div>
                );
              })}
        ...
        {/* ✅ Actions — Block AND Unblock both shown based on state */}
...
const AdminDashboard = () => {
  ...
  const confirmBlock = async () => {
    ...
    if (result.success) {
      ...
      // ✅ Update profileUser state too so modal reflects immediately
  ...
  if (authState === "loading") return <div className="min-h-screen bg-surface-base flex items-center justify-center"><Shield size={40} className="text-ink-500 animate-pulse" /></div>;
  ...
  if (loading) return <div className="min-h-screen bg-surface-base flex items-center justify-center"><div className="text-center"><Shield size={40} className="text-ink-500 mx-auto mb-4 animate-pulse" /><p className="text-gray-400">Loading admin panel...</p></div></div>;
    ...
    <div className="min-h-screen bg-surface-base">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-2xl font-display font-bold text-white">Admin Panel</h1><p className="text-xs text-gray-500">{currentUser?.email}</p></div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLockPanel} className="text-xs text-gray-500 hover:text-white border border-surface-border px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"><Lock size={12} /> Lock Panel</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
          ...
          <StatCard icon={Download} label="Downloads"       value={stats?.totalDownloads || 0}   color="bg-violet-600" />
          ...
          <StatCard icon={Clock}    label="Login Locked"    value={stats?.loginLockedUsers || 0} color="bg-orange-600" />
        </div>
          ...
          <div>
            <div className="flex flex-wrap gap-3 mb-4">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input value={noteSearch} onChange={(e) => setNoteSearch(e.target.value)} placeholder="Search notes..." className="w-full bg-surface-card border border-surface-border rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" />
              </div>
              <select value={subjectFilter} onChange={(e) => setSubjectFilter(e.target.value)} className="bg-surface-card border border-surface-border rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-ink-500">
            ...
            <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">
              <div className="grid grid-cols-12 px-4 py-3 border-b border-surface-border text-xs font-semibold text-gray-500 uppercase tracking-wide">
                <div className="col-span-4">Note</div><div className="col-span-2">Subject</div><div className="col-span-2">By</div><div className="col-span-1 text-center">DL</div><div className="col-span-2">Date</div><div className="col-span-1 text-center">Del</div>
              </div>
              {filteredNotes.length === 0
                ...
                : filteredNotes.map((note, idx) => {
                  const ft = getFileTypeLabel(note.fileType);
                  return (
                    <div key={note.id} className={`grid grid-cols-12 px-4 py-3.5 items-center hover:bg-surface-hover transition-colors ${idx !== filteredNotes.length - 1 ? "border-b border-surface-border" : ""}`}>
                      <div className="col-span-4 flex items-center gap-2 min-w-0"><span className={`text-xs font-mono font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${ft.color}`}>{ft.label}</span><p className="text-sm text-white truncate">{note.title}</p></div>
                      <div className="col-span-2"><span className="text-xs text-gray-400 bg-surface-elevated px-2 py-0.5 rounded">{note.subjectDisplay || note.subject}</span></div>
                      <div className="col-span-2"><p className="text-xs text-gray-400 truncate">{note.uploaderName || note.username || "—"}</p></div>
                      <div className="col-span-1 text-center"><span className="text-xs text-teal-400">{note.downloadCount || 0}</span></div>
                      <div className="col-span-2"><p className="text-xs text-gray-600">{formatDate(note.createdAt)}</p></div>
                      <div className="col-span-1 flex justify-center gap-1">
                        <a href={note.fileURL} target="_blank" rel="noreferrer" className="p-1.5 text-gray-600 hover:text-ink-400 rounded-lg transition-all"><ExternalLink size={13} /></a>
                        <button onClick={() => setDeleteTarget({ type: "note", data: note })} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"><Trash2 size={13} /></button>
                      </div>
                    </div>
                  );
                })}
        ...
        {activeTab === "users" && (
          <div>
            <div className="relative mb-4">
              <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
              <input value={userSearch} onChange={(e) => setUserSearch(e.target.value)} placeholder="Search by username, email or ID..." className="w-full bg-surface-card border border-surface-border rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-ink-500 transition-all" />
            </div>
            <p className="text-xs text-gray-600 mb-3">{filteredUsers.length} users</p>
            <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">

              {/* ✅ Joined column in header */}
              <div className="grid grid-cols-12 px-4 py-3 border-b border-surface-border text-xs font-semibold text-gray-500 uppercase tracking-wide">
                ...
                <React.Fragment key={user.id}>
                  <div className={`grid grid-cols-12 px-4 py-3.5 items-center hover:bg-surface-hover transition-colors ${idx !== filteredUsers.length - 1 ? "border-b border-surface-border" : ""}`}>
                    ...
                    <div className="col-span-2 flex items-center gap-2 cursor-pointer" onClick={() => setProfileUser(user)}>
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs font-bold flex-shrink-0 overflow-hidden" style={{ backgroundColor: user.avatarColor || "#3a5aff" }}>
                        {user.photoURL ? <img src={user.photoURL} alt="" className="w-full h-full object-cover" /> : user.username?.[0]?.toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm text-white truncate hover:text-ink-300 transition-colors">{user.username}</p>
                        {user.isBlocked && <span className="text-xs text-red-400 flex items-center gap-0.5"><Ban size={9} /> blocked</span>}
                      </div>
                    </div>
                    ...
                    <div className="col-span-1 text-center"><span className="text-xs text-ink-400 font-semibold">{getUserNotes(user.id).length}</span></div>
                    <div className="col-span-1 text-center"><span className="text-xs text-teal-400">{getUserDownloads(user.id)}</span></div>

                    {/* ✅ Joined date in table row */}
                    ...
                    <div className="col-span-1 flex justify-center items-center gap-1">
                      <button onClick={() => setProfileUser(user)} className="p-1.5 text-gray-600 hover:text-ink-400 hover:bg-ink-400/10 rounded-lg transition-all"><Eye size={13} /></button>
                      {user.isBlocked
                        ? <button onClick={() => handleUnblock(user)} className="p-1.5 text-gray-600 hover:text-green-400 hover:bg-green-400/10 rounded-lg transition-all" title="Unblock"><UserCheck size={13} /></button>
                        : <button onClick={() => handleBlock(user)}   className="p-1.5 text-gray-600 hover:text-amber-400 hover:bg-amber-400/10 rounded-lg transition-all" title="Block"><Ban size={13} /></button>}
                      <button onClick={() => { setDeleteTarget({ type: "user", data: user }); setDeleteConfirmText(""); }} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"><Trash2 size={13} /></button>
                    </div>
                  </div>
                </React.Fragment>
              ))}
        ...
        {activeTab === "locked" && (
          <div>
            <div className="mb-4 p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
              <p className="text-xs text-amber-400">These accounts are locked due to 5+ failed login attempts. You can unlock them before the 36-hour timer expires.</p>
            </div>
            <p className="text-xs text-gray-600 mb-3">{lockedUsers.length} locked account{lockedUsers.length !== 1 ? "s" : ""}</p>
            <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden">
              <div className="grid grid-cols-12 px-4 py-3 border-b border-surface-border text-xs font-semibold text-gray-500 uppercase tracking-wide">
              ...
              {lockedUsers.length === 0
                ? <div className="text-center py-12 text-gray-500 text-sm">No login-locked accounts 🎉</div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-surface-card border border-surface-border rounded-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setBlockTarget(null)} className="text-gray-600 hover:text-white transition-colors"><X size={18} /></button>
            </div>
            <h3 className="text-lg font-display font-bold text-white mb-1">{blockTarget.action === "block" ? "Block User?" : "Unblock User?"}</h3>
            <p className="text-sm text-gray-400 mb-1"><span className="text-white font-medium">"{blockTarget.user.username}"</span></p>
            ...
            {blockTarget.action === "block"
              ...
              : <p className="text-xs text-green-400 mb-6 flex items-center gap-1 mt-2"><UserCheck size={11} /> User will regain full access to their account.</p>}
            <div className="flex gap-3">
              <button onClick={() => setBlockTarget(null)} className="flex-1 py-2.5 border border-surface-border text-gray-400 hover:text-white rounded-xl text-sm font-medium transition-colors">Cancel</button>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-surface-card border border-surface-border rounded-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              <div className="w-10 h-10 bg-red-400/10 rounded-xl flex items-center justify-center"><AlertTriangle size={20} className="text-red-400" /></div>
              <button onClick={() => { setDeleteTarget(null); setDeleteConfirmText(""); }} className="text-gray-600 hover:text-white transition-colors"><X size={18} /></button>
            </div>
            {deleteTarget.type === "note" ? (
              <><h3 className="text-lg font-display font-bold text-white mb-1">Delete Note?</h3><p className="text-sm text-gray-500 mb-1">"{deleteTarget.data.title}"</p><p className="text-xs text-gray-600 mb-6">By {deleteTarget.data.uploaderName || deleteTarget.data.username}</p></>
            ) : (
              <><h3 className="text-lg font-display font-bold text-white mb-1">Delete User?</h3><p className="text-sm text-gray-500 mb-1">"{deleteTarget.data.username}"</p>
                ...
                <p className="text-xs text-gray-500 mb-2">Type <span className="text-red-400 font-mono font-bold">DELETE</span> to confirm:</p>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setDeleteTarget(null); setDeleteConfirmText(""); }} className="flex-1 py-2.5 border border-surface-border text-gray-400 hover:text-white rounded-xl text-sm font-medium transition-colors">Cancel</button>
