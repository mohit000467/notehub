    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 page-enter">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-ink-500 hover:bg-ink-400 text-white font-semibold rounded-xl transition-colors"
        >
          <Upload size={16} />
          Upload Notes
        </button>
      ...
      <div className="bg-ink-500/5 border border-ink-500/20 rounded-xl px-5 py-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-surface-elevated border border-surface-border hover:border-ink-600 text-gray-400 hover:text-white rounded-lg text-sm transition-all"
          >
            <Copy size={14} /> Copy ID
          </button>
          <button
            onClick={() => navigate(`/profile/${currentUser?.uid}`)}
            className="px-4 py-2 border border-surface-border hover:border-ink-600 text-gray-400 hover:text-white rounded-lg text-sm transition-all"
          >
            View Profile →
          </button>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <form onSubmit={handleUserSearch} className="relative">
            ...
            <input
              ...
              className="w-full bg-surface-elevated border border-surface-border rounded-lg pl-9 pr-16 py-2.5 text-sm text-white placeholder-gray-600 font-mono outline-none focus:border-accent-cyan transition-all"
            />
            <button
              type="submit"
              disabled={userSearching}
              className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 bg-accent-cyan/20 hover:bg-accent-cyan/30 text-accent-cyan text-xs font-semibold rounded-md transition-colors"
            >
              {userSearching ? "..." : "Find"}
            </button>
          </form>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard icon={Download} label="Total Downloads" value={totalDownloads}
          color="text-accent-cyan" bg="bg-accent-cyan/10" />
        <StatCard icon={Star} label="Avg Rating" value={avgRating}
          color="text-accent-amber" bg="bg-accent-amber/10" />
        <StatCard icon={BookOpen} label="Rated Notes" value={ratedNotes.length}
          ...
          <div className="bg-surface-card border border-surface-border rounded-xl overflow-hidden">
            ...
            {notes.map((note, idx) => {
              ...
              return (
                <div
                  key={note.id}
                  className={`grid grid-cols-12 px-4 py-4 items-center hover:bg-surface-hover transition-colors ${
                ...
                >
                  ...
                  <div className="col-span-1 flex justify-center">
                    <button
                      ...
                      className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-red-400/10 rounded transition-all"
                    >
                      <Trash2 size={15} />
                    </button>
                  </div>
